  ...
  implements IWyCloudFileSystem
{
  ...
  public IWyFileSystem.WyCommand getPreviewAddress(String paramString, IWyFileSystem.IWyCallback<IWyFileSystem.PreviewAdderss> paramIWyCallback)
  {
    ...
    paramIWyCallback = new WyCommandImpl(paramIWyCallback);
    SdkContext.getInstance().getSender().sendRequest("OfflineFilleHandleSvr.pb_weiyun_CMD_REQ_STORE_FILE_PREVIEW-19", paramString.toByteArray(), new BaseCallbackImpl(paramIWyCallback)
    {
      protected void handleSuccess(IWyFileSystem.IWyCallback<IWyFileSystem.PreviewAdderss> paramAnonymousIWyCallback, Cmd0X31B.StoreFilePreviewRspBody paramAnonymousStoreFilePreviewRspBody)
      {
        byte[] arrayOfByte1 = null;
        byte[] arrayOfByte2 = null;
        if (paramAnonymousStoreFilePreviewRspBody.filesha.has()) {
          arrayOfByte1 = paramAnonymousStoreFilePreviewRspBody.filesha.get().toByteArray();
        }
        if (paramAnonymousStoreFilePreviewRspBody.downloadkey.has()) {
          arrayOfByte2 = paramAnonymousStoreFilePreviewRspBody.downloadkey.get().toByteArray();
        }
        paramAnonymousIWyCallback.onSucceed(new IWyFileSystem.PreviewAdderss(paramAnonymousStoreFilePreviewRspBody.downloadip.get(), paramAnonymousStoreFilePreviewRspBody.downloaddns.get(), paramAnonymousStoreFilePreviewRspBody.downloadport.get(), arrayOfByte1, arrayOfByte2, paramAnonymousStoreFilePreviewRspBody.cookie.get()));
      }
    });
    return paramIWyCallback;
  }
    ...
    SdkContext.getInstance().getSender().sendRequest("OfflineFilleHandleSvr.pb_weiyun_CMD_REQ_OFFLINE_FILE-257", localReqBody.toByteArray(), new BaseCallbackImpl(paramIWyCallback)
    {
      ...
      protected void handleSuccess(IWyFileSystem.IWyCallback<IWyFileSystem.ListOfflineFile> paramAnonymousIWyCallback, Cmd0X31B.GetOfflineFileRspBody paramAnonymousGetOfflineFileRspBody)
      {
        ...
        for (;;)
        {
          ...
          {
            paramAnonymousGetOfflineFileRspBody = (Cmd0X31B.GetOfflineFileRspBody)localObject;
            if (localList2 != null) {
...
/* Location:              E:\apk\QQ_91\classes3-dex2jar.jar!\com\weiyun\sdk\impl\DefaultCloudFileSystem.class
